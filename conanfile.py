...
class NSSConan(ConanFile):
    ...
    def source(self):
        ...
        if self.settings.os == "Linux":
            tools.replace_in_file(os.path.join(self._source_subfolder, "nss", "lib", "softoken", "config.mk"),
                '	-l$(SQLITE_LIB_NAME) \\',
                '	-l$(SQLITE_LIB_NAME) -lm \\')
    ...
    def build(self):
        ...
        with tools.chdir(os.path.join(self._source_subfolder, "nss")):
            with tools.vcvars(self) if self.settings.compiler == "Visual Studio" else tools.no_op():
                # TODO: investigate why this is required to build on Centos7
